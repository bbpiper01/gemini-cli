...
/**
 * Helper to check if an error is a file-not-found error.
 */
...
/**
 * Extracts the sessionId from a file.
 * First tries to read just the first line (common case for JSONL).
 * Falls back to reading the entire file for legacy pretty-printed JSON.
 * Returns undefined if file cannot be read or parsed.
 */
async function extractSessionIdFromFile(
  filePath: string,
): Promise<string | undefined> {
  try {
    // Fast path: read first line only
    ...
    // Fallback: read entire file for legacy pretty-printed JSON
    ...
    // Silently return undefined - file read/parse failed
  }
  return undefined;
}
    ...
    for (const sessionToDelete of sessionsToDelete) {
      try {
        ...
        if (shortId) {
          ...
          // Parallel extraction: read all sessionIds at once
          const fileSessionIds = await Promise.all(
            matchingFiles.map(async (file) => ({
              file,
              sessionId: await extractSessionIdFromFile(
          ...
          for (const { file, sessionId } of fileSessionIds) {
